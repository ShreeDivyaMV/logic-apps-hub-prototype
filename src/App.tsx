...
type View = 'Overview' | 'Business Flows' | 'Logic Apps' | 'Run Details' | 'Alerts' | 'Compare' | 'Custom Query' | 'AI Insights' | 'Configuration'
type IconName = 'grid' | 'business' | 'flow' | 'runs' | 'alert' | 'compare' | 'query' | 'ai' | 'settings' | 'search' | 'refresh' | 'chevron' | 'close' | 'menu' | 'spark' | 'clock' | 'check'
...
function App() {
  ...
  const navItems: { label: View; icon: IconName; count?: number }[] = [
    { label: 'Overview', icon: 'grid' }, { label: 'Business Flows', icon: 'business', count: businessFlowCount }, { label: 'Logic Apps', icon: 'flow', count: new Set(workflows.map((workflow) => workflow.logicAppName)).size },
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="page">
          <div className="page-heading"><div><p className="eyebrow">OPERATIONS CENTER</p><h1>{activeView === 'Business Flows' ? 'Business flows' : activeView === 'Logic Apps' ? 'Logic Apps inventory' : activeView === 'Run Details' ? 'Run details' : activeView === 'Alerts' ? 'Alerts & issues' : activeView === 'Compare' ? 'Workflow comparison' : activeView === 'Custom Query' ? 'Custom query editor' : activeView === 'AI Insights' ? 'AI insights' : activeView === 'Configuration' ? 'Hub configuration' : 'Integration estate overview'}</h1><p>{activeView === 'Overview' ? 'A unified snapshot of Logic Apps, business outcomes, runs, and operational health.' : activeView === 'Business Flows' ? 'Connect workflows into end-to-end business outcomes and monitor them as one.' : activeView === 'Run Details' ? 'Analyze execution health, performance, failures, and individual run results.' : activeView === 'Custom Query' ? 'Explore fictional Application Insights telemetry and save reusable monitoring views.' : activeView === 'AI Insights' ? 'Use customer-selected GitHub models to surface explainable operational patterns.' : activeView === 'Configuration' ? 'Configure telemetry, business flow behavior, access, and security policies.' : `Monitor and manage ${activeView.toLowerCase()} across every connected subscription.`}</p></div><div className="page-heading-actions">{activeView !== 'Custom Query' && <button className="open-query-button" onClick={openWithQueryEditor}><Icon name="query" size={14}/> Open with query editor</button>}<div className="live-pill"><span /> Live monitoring</div></div></div>
          ...
          {activeView === 'Overview' && <>
            <section className="kpi-grid" aria-label="Key metrics">
              <article className="kpi-card"><div className="kpi-icon purple"><Icon name="flow" /></div><div><span>Total workflows</span><strong>24</strong><small><b>+3</b> this month</small></div></article>
              <article className="kpi-card"><div className="kpi-icon green"><Icon name="check" /></div><div><span>Success rate</span><strong>98.7%</strong><small><b>↑ 0.6%</b> vs. yesterday</small></div></article>
              <article className="kpi-card"><div className="kpi-icon blue"><Icon name="spark" /></div><div><span>Runs today</span><strong>5,263</strong><small><b>↑ 12.4%</b> vs. yesterday</small></div></article>
              <article className="kpi-card"><div className="kpi-icon amber"><Icon name="clock" /></div><div><span>Avg. duration</span><strong>2.4s</strong><small className="neutral">0.2s faster</small></div></article>
            </section>
            <section className="overview-grid">
              <article className="panel volume-panel"><div className="panel-heading"><div><h2>Run volume</h2><p>Executions across all workflows</p></div><select aria-label="Run volume period"><option>Last 24 hours</option><option>Last 7 days</option></select></div><div className="chart-summary"><strong>5,263</strong><span>total runs</span><em>+12.4%</em></div><div className="bar-chart" aria-label="Run volume chart">{runVolume.map((value, index) => <div key={index} className="bar-wrap"><div className="bar" style={{ height: `${(value / Math.max(...runVolume)) * 100}%` }} /></div>)}</div><div className="chart-axis"><span>12 AM</span><span>6 AM</span><span>12 PM</span><span>6 PM</span><span>Now</span></div></article>
              <article className="panel health-panel"><div className="panel-heading"><div><h2>Estate health</h2><p>Current workflow status</p></div><button onClick={() => setActiveView('Logic Apps')}>View all <Icon name="chevron" size={14} /></button></div><div className="donut-row"><div className="donut"><div><strong>24</strong><span>workflows</span></div></div><ul><li><span className="legend healthy" />Healthy <strong>19</strong></li><li><span className="legend warning" />Warning <strong>3</strong></li><li><span className="legend critical" />Critical <strong>1</strong></li><li><span className="legend disabled" />Disabled <strong>1</strong></li></ul></div></article>
            </section>
            <section className="panel business-flow-snapshot"><div className="panel-heading"><div><h2>Business Flows snapshot</h2><p>End-to-end outcomes across workflows and Logic Apps</p></div><button onClick={() => setActiveView('Business Flows')}>Open Business Flows <Icon name="chevron" size={14} /></button></div><div className="snapshot-grid"><div className="snapshot-summary"><div className="snapshot-ring"><div><strong>{initialBusinessFlows.length}</strong><span>flows</span></div></div><div className="snapshot-stats"><span><i className="healthy"/>2 healthy</span><span><i className="critical"/>1 needs attention</span><p><strong>{new Set(initialBusinessFlows.flatMap((flow) => flow.workflowIds)).size}</strong> workflows across <strong>{new Set(workflows.map((workflow) => workflow.logicAppName)).size}</strong> Logic Apps</p></div></div><div className="snapshot-flows">{initialBusinessFlows.map((flow) => { const members = workflows.filter((workflow) => flow.workflowIds.includes(workflow.id)); const hasCritical = members.some((workflow) => workflow.health === 'Critical'); const hasWarning = members.some((workflow) => workflow.health === 'Warning'); const flowStatus = hasCritical ? 'Critical' : hasWarning ? 'Warning' : 'Healthy'; return <button key={flow.id} onClick={() => setActiveView('Business Flows')}><span className={`snapshot-status ${flowStatus.toLowerCase()}`}><i/></span><span><strong>{flow.name}</strong><small>{members.length} workflows · {new Set(members.map((member) => member.logicAppName)).size} Logic Apps</small></span><b>{members.reduce((total, member) => total + member.runs24h, 0).toLocaleString()}<small> runs</small></b><Icon name="chevron" size={14}/></button> })}</div></div></section>
          </>}
